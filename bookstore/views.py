...
def request_form(request):
	...
	return render(request, 'publisher/delete_request.html', {'books': books})
...
def uabook(request):
    if request.method == 'POST':
        try:
            ...
            if not all([title, author, publisher, year, desc, pdf]):
...
class UCreateChat(LoginRequiredMixin, CreateView):
	...
	def get_context_data(self, **kwargs):
		...
		context['chat_messages'] = Chat.objects.all().order_by('-posted_at')
		return context
